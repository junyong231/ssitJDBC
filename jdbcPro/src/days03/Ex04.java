		...
		*/
		...
		//풀이
			      ...
			      try {
			         ...
			         while (deptRs.next()) {
			        	 empList = null; //초기화 . 안하면 사원이 null일 때 전에 있던 정보들 그대로 put
	   ...
	   private static void dispLHMap(LinkedHashMap<DeptVO, ArrayList<EmpVO>> lhMap) {
		      ...
		      while (eir.hasNext()) {
		         Entry<DeptVO, ArrayList<EmpVO>> entry = eir.next();
		          ...
		          ArrayList<EmpVO> empList = entry.getValue();  
		          if (empList == null) { //사원이 없을 때
						System.out.println("\t 해당하는 부서원 없음");
						continue; // 출력 부분 패스하고 다음 while조건으로 넘어감
					}
		          Iterator<EmpVO> ir = empList.iterator();
		          while (ir.hasNext()) {
		            EmpVO evo =  ir.next();
